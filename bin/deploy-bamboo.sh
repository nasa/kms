...
dockerRun() {
  docker run \
        ...
        --env "KMS_CACHE_TTL_SECONDS=${bamboo_KMS_CACHE_TTL_SECONDS:-3600}" \
        --env "KMS_CACHE_CLUSTER_SIZE_GB=${bamboo_KMS_CACHE_CLUSTER_SIZE_GB:-0.5}" \
        --env "KMS_CACHE_CLUSTER_ENABLED=${bamboo_KMS_CACHE_CLUSTER_ENABLED:-true}" \
    $dockerTag "$@"
}
