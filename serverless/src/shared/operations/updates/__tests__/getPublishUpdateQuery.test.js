...
describe('getPublishUpdateQuery', () => {
  ...
  describe('when validation errors occur', () => {
    test('should throw an error for an invalid name', () => {
      expect(() => getPublishUpdateQuery(123, '2023-06-01T12:00:00Z')).toThrow('Invalid name provided')
    })

    test('should throw an error for an invalid updateDate', () => {
      expect(() => getPublishUpdateQuery('v1.0.0', 123)).toThrow('Invalid updateDate provided')
    })
  })
