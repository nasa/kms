...
describe('formatPath', () => {
  describe('platforms, instruments, projects schemes', () => {
    ...
    schemes.forEach((scheme) => {
      test(`should return the path as is when path length equals maxLevel for ${scheme}`, () => {
        const result = formatCsvPath(scheme, 5, ['a', 'b', 'c'], false)
        expect(result).toEqual(['a', 'b', 'c'])
      })

      test(`should pad the path with spaces when path length is less than maxLevel and not a leaf for ${scheme}`, () => {
        const result = formatCsvPath(scheme, 6, ['a', 'b'], false)
        expect(result).toEqual(['a', 'b', ' ', ' '])
      })

      test(`should insert a space at maxLevel - 2 when path length is less than maxLevel and is a leaf for ${scheme}`, () => {
  ...
  describe('sciencekeywords, chronounits, locations, discipline, rucontenttype, measurementname schemes', () => {
    ...
    schemes.forEach((scheme) => {
      ...
      test(`should pad the path with spaces when path length is less than maxLevel for ${scheme}`, () => {
