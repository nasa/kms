...
const fixturePath = path.resolve(
  rootDir,
  'scripts/local/fixtures/metadata_correction_service.iso-smap.corrections.json'
)
const metadataPayloadPath = path.resolve(
  rootDir,
  'scripts/local/fixtures/metadata_correction_service.iso-smap.example.xml'
)
const outputPath = path.resolve(
  rootDir,
  'tmp/metadata_correction_service_iso-smap_smoke_output.xml'
)

const fixture = JSON.parse(await fs.readFile(fixturePath, 'utf8'))
const metadataPayload = await fs.readFile(metadataPayloadPath, 'utf8')

const { applyIsoSmapMetadataCorrections } = await import('../../serverless/src/shared/applyIsoSmapMetadataCorrections')
...
const correctionResult = await applyIsoSmapMetadataCorrections(request)
