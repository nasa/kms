 ...
 */
async function loadConcepts(token, versionType, version, versionName, batchSize = 1000) {
  // Read the XML file
...
async function loadSchemes(token, versionType, version, versionName) {
  // Read the XML file
...
async function recreateDatabase(token) {
  const response = await fetch('https://cmr.sit.earthdata.nasa.gov/kms/recreateDatabase', {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/rdf+xml',
      Accept: 'application/rdf+xml',
    ...
    for (const versionType of versionTypes) {
      ...
      for (const version of versions) {
        try {
          ...
          await loadConcepts(token, versionType, version, versionName)
          console.log('All concepts loaded successfully ', versionType, version, versionName)
          await loadSchemes(token, versionType, version, versionName)
          console.log('All schemes loaded successfully', versionType, version, versionName)
        } catch (error) {
